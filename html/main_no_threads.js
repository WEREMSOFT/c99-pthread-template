...
var wasmBinaryFile = 'main_no_threads.wasm';
...
var ASM_CONSTS = {
  1982: function() {postMessage({cmd : 'processQueuedMainThreadWork'})},  
 2033: function($0) {if (!ENVIRONMENT_IS_PTHREAD) { if (!PThread.pthreads[$0] || !PThread.pthreads[$0].worker) { return 0; } PThread.pthreads[$0].worker.postMessage({cmd : 'processThreadQueue'}); } else { postMessage({targetThread : $0, cmd : 'processThreadQueue'}); } return 1;},  
 2401: function() {return !!(Module['canvas'])},  
 2437: function() {noExitRuntime = true}
};
...
// {{PRE_LIBRARY}}
  ...
  var __main_thread_futex_wait_address=6048;function _emscripten_futex_wake(addr, count) {
  ...
  function __cleanup_thread(pthread_ptr) {
      ...
      if (pthread) {
        ...
        if (typeof SharedArrayBuffer === 'undefined') return []; // No multithreading support, no-op.
        var pthreadMainJs = "main_no_threads.worker.js";
